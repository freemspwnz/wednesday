...
- **Контейнер зависимостей `BotServices`:** Все основные сервисы собираются в единый dataclass `BotServices`, который передается во все обработчики и компоненты. Контейнер включает:
  - `image_generator`: Генератор изображений
  - `scheduler`: Планировщик задач (опционально)
  - `usage`: Трекер использования
  - `chats`: Хранилище чатов
...
- **Настройки через `AppSettings`:** Инкапсулирует основные настройки приложения (например, `admin_chat_id`, `chat_id`, `scheduler_send_times`, параметры rate limiting), доступные через `BotServices.settings`. Инициализируется один раз при создании бота из глобального `Config`.
- **В BotRunner:** Сервисы создаются напрямую в конструкторах (`WednesdayBot.__init__()`, `SupportBot.__init__()`), собираются в `BotServices` и передаются в обработчики
- **В обработчиках:** Все зависимости доступны через `self.services` (экземпляр `BotServices`), передаваемый в конструкторе
...
**Минимальный BotServices для SupportBot:**
- `SupportBot` использует минимальный `BotServices` с только необходимыми полями (`settings` и `rate_limiter`)
- Остальные поля устанавливаются в `None`, так как `SupportBot` не использует полный функционал основного бота
- Это позволяет `SupportBot` наследоваться от `BaseHandlers` и использовать общие методы, сохраняя минимальные зависимости
