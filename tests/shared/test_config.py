...
def test_config_creation_from_env(monkeypatch: Any) -> None:
...
def test_config_model_validate(monkeypatch: Any) -> None:
...
def test_config_secret_file_support(monkeypatch: Any, tmp_path: Any) -> None:
...
def test_config_direct_access(monkeypatch: Any) -> None:
    ...
    # Тестируем прямой доступ к конфигурации
    assert config.gigachat.authorization_key == "test-key"
    assert config.kandinsky.api_key == "test-api"
    assert config.kandinsky.secret_key == "test-secret"
    assert config.retry.standard_max_attempts == 3  # default
    assert config.circuit_breaker.threshold == 5  # default
