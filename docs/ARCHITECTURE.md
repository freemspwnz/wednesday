...
Все инфраструктурные зависимости (PostgreSQL pool, Redis client)
инициализируются явно в `main.py` и передаются через Dependency Injection
во все сервисы.

**ВАЖНО:** Функции `_get_postgres_pool()` и `_get_redis()` удалены. Используйте Dependency Injection для получения пулов и клиентов.
Всегда получайте пулы/клиенты через DI из `container.py` или `main.py`.

### Порядок инициализации:

1. `main.py` → `_init_and_validate_infrastructure()`
   - Инициализирует и валидирует Postgres pool
   - Инициализирует и валидирует Redis client (с fallback на in-memory)

2. `main.py` → `build_bot()`
   - Передаёт пулы явно в `build_bot_services()`

3. `container.py` → `build_bot_services()`
   - Создаёт все сервисы с явными зависимостями
   - Передаёт пулы во все репозитории и сервисы
...
- **Контейнер зависимостей `BotServices`:** Все основные сервисы собираются в единый dataclass `BotServices`, который передается во все обработчики и компоненты. Финальный контракт контейнера включает:
  ...
  - `postgres_pool`: Пул подключений PostgreSQL (ОБЯЗАТЕЛЬНЫЙ, для прямого доступа)
  - `redis_client`: Redis-клиент (ОБЯЗАТЕЛЬНЫЙ, для прямого доступа)
...
- **Явная передача инфраструктурных зависимостей:** Все функции сборки (`build_bot`, `build_bot_services`, `build_image_stack`, `build_admin_dashboard_service`) принимают `db_pool` и `redis_client` явно через параметры, а не через глобальные функции. Функции `_get_postgres_pool()` и `_get_redis()` полностью удалены. Все зависимости передаются через Dependency Injection, что обеспечивает прозрачность зависимостей и упрощает тестирование.
