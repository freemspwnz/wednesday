...
- **Улучшено соблюдение Dependency Injection в Celery context**:
  - Добавлены параметры `pool_factory` и `redis_factory` в функции `get_services_context()` и `_ensure_pools_initialized()`
  - Фабрики теперь передаются через параметры вместо создания внутри функций
  - Сохранена обратная совместимость: фабрики создаются автоматически, если не переданы
  ...
  - Обновлена задача `daily_statistics_task` для использования `get_services_context()` вместо приватной функции
  - Улучшена тестируемость: можно передавать моки фабрик для тестирования
  - Улучшено соблюдение принципов SOLID: зависимости передаются явно
