...
Все константы должны быть типизированы через `Final` для явного указания, что они не изменяются:
...
**Важно:** Используйте `Final` для всех констант верхнего уровня (модульные константы), чтобы mypy мог статически проверить, что они не переприсваиваются.
...
### 8. Docstrings и type hints: не дублируйте информацию

**Важно:** Docstrings должны описывать **поведение** и **бизнес-логику**, а не дублировать информацию из type hints. Type hints описывают **структуру** (типы параметров и возвращаемых значений), а docstrings описывают **что делает** метод.

```python
# ✅ Хорошо: docstring описывает поведение, не дублирует типы
async def check_api_status(
    self,
    save_models: bool = True,
) -> tuple[bool, str, list[str], tuple[str | None, str | None]]:
...
# ❌ Плохо: docstring дублирует информацию из type hints
async def check_api_status(
    self,
    save_models: bool = True,
) -> tuple[bool, str, list[str], tuple[str | None, str | None]]:
    ...
    Returns:
        tuple[bool, str, list[str], tuple[str | None, str | None]] - кортеж с результатами.
    """
```

**Правила написания docstrings:**

1. **Не дублируйте типы**: Если тип уже указан в type hints, не нужно повторять его в docstring
2. **Описывайте поведение**: Объясняйте, что делает метод, какие побочные эффекты, какие условия
3. **Указывайте исключения**: Документируйте возможные исключения в секции `Raises`
4. **Давайте примеры**: Для сложных методов добавляйте примеры использования
5. **Краткие описания**: Для простых методов достаточно краткого описания

**Примеры для разных случаев:**

```python
# Простой метод - краткое описание
def _clean_prompt(prompt: str) -> str:
    """Очищает промпт от лишних символов, форматирования и маркеров."""
    ...

# Метод с параметрами - описываем назначение параметров
async def set_model(self, model_name: str) -> tuple[bool, str]:
    ...
    Если найдено несколько моделей, возвращается ошибка с предложением
    уточнить название или использовать ID.

    Args:
        model_name: ID модели или часть названия для поиска.
...
# Метод с исключениями - документируем все возможные исключения
async def generate(self, prompt: str, user_id: str | None = None) -> bytes | None:
    ...
    ожидание завершения и получение результата. При любой ошибке возвращает None
    и логирует детали ошибки.
...
# ✅ Хорошо: константа типизирована через Final
HTTP_STATUS_OK: Final[int] = 200
MAX_RETRIES: Final[int] = 3
DEFAULT_TIMEOUT: Final[float] = 30.0

# ❌ Плохо: константа не типизирована
HTTP_STATUS_OK = 200
```
