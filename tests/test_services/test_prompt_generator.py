...
@pytest.mark.asyncio
async def test_prompt_storage_writes_normalized_prompt_without_extra_quotes(tmp_path: Path, monkeypatch: Any) -> None:
    """
    ...
    monkeypatch.setattr(
        "services.infrastructure.storage.prompt_storage.get_logger", lambda *_args, **_kwargs: fake_logger
    )
...
@pytest.mark.asyncio
async def test_prompt_storage_preserves_multiline_prompt_without_outer_spaces(tmp_path: Path, monkeypatch: Any) -> None:
    """
    ...
    path_str = await storage.save(multiline_prompt, source="multiline")
    assert path_str is not None
...
@pytest.mark.asyncio
async def test_prompt_storage_empty_prompt_raises_and_logs_warning(tmp_path: Path, monkeypatch: Any) -> None:
    """
    ...
    with pytest.raises(ValueError):
        await storage.save("   ", source="empty")

    fake_logger.warning.assert_called_once()


@pytest.mark.asyncio
async def test_prompt_storage_works_with_tmpfs_like_volume(tmp_path: Path, monkeypatch: Any) -> None:
    """
    ...
    storage = PromptStorageService(base_dir=volume_dir)

    path_str = await storage.save("A frog from tmpfs", source="tmpfs")
    assert path_str is not None
