...
- **Устранение блокирующего файлового I/O в асинхронном коде**:
  ...
  - Вспомогательный метод `_send_fallback_image()` теперь использует `_get_random_saved_image_async()` вместо синхронного чтения файлов, что делает отправку fallback‑картинок дружелюбнее к асинхронному циклу событий.
