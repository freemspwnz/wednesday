...
- **Реализация рекомендаций из INFRA_LAYER_AUDIT.md по явной передаче пулов через DI**:
  ...
  - Добавлен параметр `pool` в функцию `ensure_schema()` для явной передачи пула при инициализации схемы БД
  ...
  - Улучшены комментарии в `healthcheck.py` для пояснения использования fallback на глобальные функции (для обратной совместимости)
  - Все функции сохраняют fallback на глобальные пулы для обратной совместимости, но основные пути используют явную передачу через DI
