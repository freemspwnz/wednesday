...
- **Устранение всех оставшихся нарушений из bot/handlers/**:
  ...
  - **admin.py**: Заменены все дублирующиеся строки ошибок доступа на константы из `messages.py`
  - **admin.py**: Заменены все проверки доступа на использование `_check_admin_access()`
  - **admin.py**: Унифицирована проверка аргументов через `_has_args()`
  - **admin.py**: Перенесен импорт `AdminNotificationBuilders` на уровень модуля
  - **models.py**: Заменены все дублирующиеся строки ошибок доступа на константы
  - **models.py**: Заменены все проверки доступа на использование `_check_admin_access()`
  - **models.py**: Унифицирована проверка аргументов через `_has_args()`
  - **user.py**: Заменено приветственное сообщение на константу из `messages.py`
  - **chat_event.py**: Заменено приветственное сообщение на константу из `messages.py`
  - **chat_event.py**: Перенесен импорт `ChatInfoService` на уровень модуля
  - Улучшено соблюдение принципа DRY: устранено дублирование кода (15 мест с ошибками доступа, 7 мест с проверкой аргументов, 2 места с приветствиями)
  - Улучшена поддерживаемость: все пользовательские сообщения централизованы в одном файле
...
- **Рефакторинг async Celery: разделение на модули и упрощение структуры**:
  ...
  - Перенесен `app.py` из `celery/async/` в корень проекта и переименован в `worker.py` для упрощения импортов
  - Упрощен `worker.py`: убрана проверка timezone из основного тела (перенесена в обработчик `worker_ready`)
  - Заменен ручной импорт задач на `autodiscover_tasks()` для автоматической регистрации
  - Вынесена логика получения Redis URL в функцию `get_celery_redis_url()` в `config.py`
  - Убраны префиксы `new_` из имен задач: `wednesday.new_send_frog` → `wednesday.send_frog`
  - Обновлены все импорты для использования нового расположения модулей

- **Рефакторинг логгера: устранена избыточная обёртка над обёрткой**:
  - Упрощена структура логирования: убрана двойная обёртка в `LoguruLogger._log()`
  - Вся логика маскировки секретов и очистки данных перенесена непосредственно в `LoguruLogger._log()`
  - Функция `log_event()` теперь является простой обёрткой, которая вызывает методы ILogger
  - Улучшена производительность: устранены лишние вызовы функций
  - Улучшена читаемость кода: логика логирования сосредоточена в одном месте
  - Сохранена обратная совместимость: интерфейс `log_event()` остался без изменений

- **Замена ImageCacheService на ImageExistenceService для проверки существования изображений**:
  - Удалён `ImageCacheService` (не выполнял свою функцию кэширования)
  - Создан `ImageExistenceService` для проверки существования изображений в постоянном хранилище
  - Обновлён `ImageGenerationCoordinator`: заменён `ICache[tuple[bytes, str]]` на `ImageExistenceService`
  - Обновлён `ImageStorageUnitOfWork`: заменён `ICache` на `ImageExistenceService` для сохранения в постоянное хранилище
  - Обновлён протокол `ICache`: улучшена документация, протокол теперь используется только для временного кэширования (промпты)
  - Улучшена архитектура: проверка существования выполняется через репозитории (PostgreSQL + FS), а не через временный кэш
  - Обновлён `src/infra/cache/__init__.py`: удалён экспорт неиспользуемого `ImageCacheService`

### Добавлено

- **Добавлен ImageExistenceService для проверки существования изображений в постоянном хранилище**:
  - Создан новый application service `ImageExistenceService` для проверки существования изображений по промптам
  - Использует репозитории `IPromptRepo` и `IImageRepo` для работы с постоянным хранилищем (PostgreSQL + FS)
...
- **Добавлен упрощенный WednesdayBot (new_wednesday_bot.py) с чистой архитектурой**:
  ...
  - Упрощен конструктор: принимает готовый `application` и `container` вместо создания компонентов внутри
  - Заменен метод `start()` на `run()`: простая последовательность регистрации хендлеров и запуска polling
  - Удалена сложная логика: нет планировщика, валидации конфигурации, хуков и уведомлений
  - Удален метод `send_wednesday_frog()`: бизнес-логика вынесена из бота
  - Упрощен метод `stop()`: только остановка Application без дополнительной логики
  - Реализует Single Responsibility Principle: бот отвечает только за Telegram-часть
  - Использует стандартный `application.run_polling()` вместо legacy паттерна
  - Все зависимости получаются через Container: бот не знает про БД/Redis напрямую

- **Добавлен улучшенный Container (new_container.py) с кэшированием и приватными фабриками**:
  - Создан новый модуль `new_container.py` с улучшенной архитектурой Composition Root
  - Реализовано кэширование `build_bot_services()` через `self._services` для предотвращения повторного создания сервисов
  - Все фабричные методы вынесены в приватные методы класса (`_build_*`, `_create_*`) вместо модульных функций
  - Добавлены `@cached_property` для инфраструктурных объектов (кэши, лимитеры, сервисы, репозитории)
  - Добавлен метод `build_handlers_registry()` для создания регистратора хендлеров через контейнер
  - Упрощен конструктор: не требует `celery_app` и `application` (передаются через build_handlers_registry)
  - Улучшена декомпозиция: все сложные сервисы собираются через приватные фабрики
  - Оптимизировано использование памяти: инфраструктурные объекты создаются один раз через cached_property
...
- **Рефакторинг клиентов для использования единой HTTP сессии через DI**:
  ...
  - Обновлен `ClientManagementService`: методы создания клиентов теперь требуют передачи сессии
  - Клиенты больше не управляют жизненным циклом HTTP сессии - она управляется извне через DI
  - Улучшено управление ресурсами: единая сессия переиспользуется между всеми клиентами
  - Улучшена архитектура: клиенты следуют принципу Dependency Injection для HTTP сессии
...
- **Унифицирована генерация ключей идемпотентности в Celery задачах**:
  ...
  - Обновлена `generate_frog_image_task` для использования helper-функции вместо ручной генерации
  - Обновлена `send_frog_manual` для использования helper-функции вместо ручной генерации
  - Устранено дублирование кода генерации ключей (принцип DRY)
  - Улучшена консистентность: все задачи используют единый подход к генерации ключей
  - Исправлен баг в `send_wednesday_frog_task`: теперь учитывается `slot_time` при генерации ключа

- **Устранено нарушение границ слоёв: удалены пулы БД из ServicesContext**:
  - Удалены `postgres_pool` и `redis_client` из `ServicesContext` TypedDict
  - Обновлена документация в `get_services_context()` и `build_celery_services_context()`
  - Пулы используются только для создания сервисов через DI (fork-safe)
  - Все операции с БД выполняются через сервисы и репозитории, а не напрямую
  - Улучшено соблюдение принципа инверсии зависимостей (DIP)
  - Улучшено соблюдение границ слоёв Clean Architecture
  - Обновлены тесты для соответствия новой структуре контекста

### Изменено

- **Устранено дублирование кода идемпотентности в Celery задачах**:
  - Создана helper-функция `_get_idempotency_service()` для получения сервиса из контекста
  - Создана helper-функция `execute_with_idempotency()` для упрощения вызова идемпотентности
  - Создана helper-функция `generate_daily_idempotency_key()` для генерации ключей на основе даты
  - Создана helper-функция `execute_daily_task_with_idempotency()` для ежедневных задач
  - Обновлены все задачи для использования helper-функций вместо дублирующегося кода
  - Улучшено соблюдение принципа DRY: логика идемпотентности централизована в helper-функциях
  - Улучшена читаемость: задачи стали короче и понятнее
  - Улучшена поддерживаемость: изменения в логике идемпотентности требуют правки только в helper-функциях

- **Оптимизирован вызов send_task() в CeleryTaskQueue для избежания блокировки event loop**:
  - Обёрнут синхронный вызов `send_task()` в `asyncio.to_thread()` в методе `send_frog_manual_task`
  - Улучшена производительность: event loop не блокируется при постановке задач в очередь
  - Улучшена масштабируемость: при высокой нагрузке не будет блокировки event loop

- **Исправлена стратегия retry в Celery задачах: устранено дублирование**:
  - Убрана ручная логика `self.retry()` из всех задач
  - Используется только `autoretry_for` для автоматического retry
  - Обновлён `autoretry_for` для всех задач: добавлен `OSError` в список retryable ошибок
  - Убрана проверка `is_retryable_error()` из задач (фильтрация происходит через `autoretry_for`)
  - Убрано ручное обновление метрик `CELERY_TASK_RETRIES_TOTAL` (Celery делает это автоматически)
  - Улучшена надёжность: предотвращён риск двойного retry
  - Упрощён код: задачи стали проще и понятнее

- **Добавлена идемпотентность для всех Celery задач**:
  - Добавлена идемпотентность для `send_wednesday_frog_task` через `IdempotencyService` с ключом на основе `slot_time` или текущей даты
  - Добавлена идемпотентность для `generate_frog_image_task` через `IdempotencyService` с ключом на основе `user_id` или текущей даты/времени
  - Добавлена идемпотентность для `daily_cleanup_task` через `IdempotencyService` с ключом на основе текущей даты (выполняется раз в день)
  - Добавлена идемпотентность для `daily_statistics_task` через `IdempotencyService` с ключом на основе текущей даты (выполняется раз в день)
  - Созданы вспомогательные функции для каждой задачи (`_send_wednesday_frog_operation`, `_generate_frog_image_operation`, `_daily_cleanup_operation`, `_daily_statistics_operation`) для использования в `IdempotencyService`
  - Улучшена надёжность: предотвращены дубликаты выполнения задач при retry или повторных запусках
  - Улучшена эффективность: задачи не выполняются повторно без необходимости

- **Устранение дублирования кода получения контекста в Celery задачах**:
  - Создан декоратор `with_services_context` для автоматического получения контекста сервисов
  - Обновлены все Celery задачи для использования декоратора вместо дублирующегося кода
  - Добавлен `@functools.wraps` для сохранения метаданных функции (критично для регистрации задач в Celery)
  - Улучшена документация декоратора с упоминанием необходимости `bind=True` в `@celery_app.task`
  - Улучшено соблюдение принципа DRY: логика получения контекста централизована в одном месте
  - Улучшена читаемость: задачи стали короче и фокусируются на бизнес-логике
  - Улучшена поддерживаемость: изменения в логике получения контекста требуют правки только в декораторе

- **Рефакторинг логики идемпотентности: вынос из инфраструктурного слоя в application-слой**:
  - Создан протокол `IIdempotencyService` в `shared/protocols.py` для абстракции логики идемпотентности
  - Создан `IdempotencyService` в `app/idempotency_service.py` для инкапсуляции логики кэширования результатов
  - Обновлена задача `send_frog_manual` для использования `IdempotencyService` вместо прямого доступа к Redis
  - Добавлен `IdempotencyService` в `build_celery_services_context()` для использования в Celery задачах
  - Обновлён `ServicesContext` для включения `idempotency_service` через протокол `IIdempotencyService`
  - Улучшено соблюдение границ слоёв: логика идемпотентности теперь в application-слое, а не в инфраструктурном
  - Улучшено соблюдение SRP: задача только оркестрирует, сервис управляет кэшированием
  - Улучшена тестируемость: логику идемпотентности можно тестировать отдельно от Celery
...
- **Полностью мигрировано на явную передачу зависимостей в Celery context**:
  ...
  - Убрана fallback логика создания фабрик внутри функций
  - Создана helper функция `create_factories()` для явного создания фабрик в Celery задачах
  - Обновлены все Celery задачи для явного создания фабрик через `create_factories()` перед вызовом `get_services_context()`
  - Вынесена логика создания Redis клиента в отдельную функцию `_get_redis_client()` для соблюдения DRY
  - Удалён прямой импорт приватной функции `_ensure_pools_initialized()` из `tasks.py`
  - Обновлены тесты для передачи фабрик явно через моки
  - Улучшена тестируемость: все зависимости передаются явно, нет скрытых зависимостей
  - Улучшено соблюдение принципов SOLID и DI: зависимости передаются явно, нет глобального состояния
...
- **Устранено нарушение границ слоёв в Celery tasks**:
  ...
  - Обновлён `infra/celery/tasks.py` для использования протоколов вместо прямых импортов из app-слоя
  - Обновлён `infra/celery/context.py` для использования протоколов в TypedDict контекста
  - Обновлён `app/frog_processing_service.py` для импорта `FrogRequestResult` из `shared/models`
  - Улучшено соблюдение принципа Dependency Inversion: инфраструктурный слой зависит от абстракций, а не от конкретных классов
  - Улучшена тестируемость: Celery tasks можно тестировать с моками протоколов без реальных сервисов
  - Улучшена архитектурная целостность: устранена зависимость infra-слоя от app-слоя
  - Сохранена обратная совместимость: поведение Celery tasks не изменилось, только внутренняя структура зависимостей

### Исправлено

- **Исправлена обработка системных исключений в доменных сервисах**:
  - Добавлена проверка системных исключений (`MemoryError`, `SystemExit`, `KeyboardInterrupt`) перед обработкой общего `Exception` в `ImageGenerationService`
  - Добавлена проверка системных исключений перед обработкой общего `Exception` в `PromptGenerationService`
  - Улучшено соблюдение принципа "fail fast": системные ошибки теперь пробрасываются без обработки
  - Улучшена корректность завершения приложения: критические системные ошибки не скрываются доменными исключениями
  - Улучшена отказоустойчивость: предотвращено некорректное оборачивание системных ошибок в доменные исключения

### Изменено

- **Добавлен Value Object UserID для инкапсуляции конвертации user_id**:
  - Создан `UserID` в `domain/value_objects.py` для инкапсуляции логики конвертации между доменным типом (int) и форматом протоколов (str)
  - Обновлён `ImageGenerationService.generate()` для использования `UserID` вместо `int | None`
  - Обновлён `ImageGenerationCoordinator` для конвертации `int` -> `UserID` при передаче в domain слой
  - Обновлены тесты для использования `UserID` Value Object
  - Улучшена инкапсуляция: логика конвертации локализована в одном месте (`UserID.for_logging()`)
  - Улучшена явность: понятно, что для логирования используется специальный метод
  - Улучшена расширяемость: можно легко добавить валидацию и другие форматы в будущем
  - Сохранена обратная совместимость: app слой продолжает работать с `int`, конвертация происходит на границе domain/app

- **Вынесена логика форматирования fallback промптов в отдельный доменный объект**:
  - Создан `FallbackPromptBuilder` для инкапсуляции бизнес-правил форматирования fallback промптов
  - Обновлён `PromptGenerationService.get_fallback_prompt()` для использования `FallbackPromptBuilder`
  - Улучшено соблюдение принципа SRP: сервис больше не содержит детали форматирования промптов
  - Улучшена тестируемость: логика форматирования может быть протестирована независимо через `random_selector`
  - Улучшена переиспользуемость: логику форматирования можно использовать в других местах
  - Улучшена расширяемость: изменения в шаблоне форматирования локализованы в одном месте
  - Сохранена обратная совместимость: поведение сервиса не изменилось, только внутренняя структура

- **Устранено нарушение границ слоёв в ChatInfoService**:
  - Расширен протокол `IMessagingService` методами `get_chat_info()` и `get_chat()` для получения информации о чатах
  - Реализованы методы в `PTBMessagingService` с graceful degradation (возвращают безопасные значения вместо проброса исключений)
  - Обновлён `ChatInfoService` для использования `IMessagingService` вместо прямого использования `telegram.Bot`
  - Обновлён DI контейнер для передачи `messaging_service` в `ChatInfoService`
  - Улучшено соблюдение принципа Dependency Inversion: app-слой зависит только от протоколов, а не от конкретных реализаций
  - Улучшена тестируемость: `ChatInfoService` можно тестировать с моками протокола без реального Telegram Bot API
  - Улучшена архитектурная целостность: устранена зависимость app-слоя от инфраструктурных деталей (telegram.Bot)
  - Сохранена обратная совместимость: поведение `ChatInfoService` не изменилось, только внутренняя реализация

### Изменено

- **Устранено избыточное использование `except Exception: pass` в обработчиках**:
  - Добавлен метод `_handle_send_message_error()` в `BaseHandlers` для централизованной обработки ошибок отправки сообщений
  - Заменены все `except Exception: pass` на специфичную обработку с разделением сетевых и неожиданных ошибок
  - Улучшено логирование: сетевые ошибки Telegram API логируются как warning, неожиданные ошибки - как error с полным стеком
  - Улучшена диагностика: все ошибки теперь логируются с контекстом, что упрощает отладку
  - Улучшено соблюдение принципа "fail fast": ошибки не скрываются, а явно обрабатываются и логируются
  - Улучшена обработка ошибок в `_safe_delete_message()`: разделена обработка сетевых и неожиданных ошибок
  - Исправлены контексты сообщений об ошибках в `handlers_models.py` для правильной идентификации проблем
  - Сохранена обратная совместимость: поведение обработчиков не изменилось, только улучшено логирование

- **Устранено дублирование логики между WednesdayBot и SupportBot**:
  - Вынесена общая логика запуска и остановки в методы `start()` и `stop()` миксина `BotLifecycleMixin`
  - Добавлен параметр `pre_startup_hook` в метод `start()` для выполнения специфичной логики перед запуском
  - Добавлены параметры `notification_builder`, `chat_id`, `pre_stop_hook`, `post_stop_hook` в метод `stop()` для гибкой настройки остановки
  - Упрощены методы `start()` и `stop()` в `WednesdayBot` и `SupportBot` - оставлена только специфичная логика
  - Улучшено соблюдение принципа DRY: общая логика жизненного цикла централизована в миксине
  - Улучшена поддерживаемость: изменения в общей логике жизненного цикла вносятся в одном месте
  - Улучшена тестируемость: общая логика жизненного цикла может быть протестирована независимо
  - Сохранена обратная совместимость: поведение ботов не изменилось, только внутренняя структура
...
- **Устранено нарушение SRP в SupportBot через вынос обработчиков команд в отдельный модуль**:
  ...
  - Убрано наследование `SupportBot` от `BaseHandlers` для соблюдения принципа единственной ответственности
  - Обновлен `SupportBotHandlersRegistry` для использования `SupportBotHandlers` вместо методов из `SupportBot`
  - Обновлен `build_support_bot_components` для создания `SupportBotHandlers` с передачей `request_start_main` callback
  - Улучшено соблюдение принципа SRP: `SupportBot` отвечает только за управление жизненным циклом, обработчики команд вынесены в отдельный класс
  - Улучшено соблюдение принципа DRY: структура аналогична основному боту с разделением на классы обработчиков
  - Улучшена тестируемость: обработчики команд можно тестировать независимо от жизненного цикла бота
  - Улучшена поддерживаемость: изменения в обработчиках команд изолированы от логики жизненного цикла

- **Добавлена защита от зависаний команд через таймаут в обработчике ошибок**:
  - Добавлен опциональный параметр `timeout` в метод `_handle_command_errors` для ограничения времени выполнения команды
  - Добавлена константа `COMMAND_TIMEOUT_DEFAULT` для будущего использования таймаутов по умолчанию
  - Реализована защита от бесконечных циклов через `asyncio.wait_for` с обработкой `TimeoutError`
  - Улучшена отказоустойчивость: команды теперь не могут зависнуть на неопределенное время
  - Сохранена обратная совместимость: таймаут опционален и по умолчанию не применяется
  - Улучшена защита от утечек памяти: таймаут предотвращает накопление ресурсов при зависших командах
  - Добавлено логирование при превышении таймаута с отправкой сообщения пользователю
...
- **Устранена утечка инфраструктурных деталей в bot-слое**:
  ...
  - Убран прямой импорт из `infra.logging.logger` в `BotErrorHandler`, используется протокол `ILogger`
...
- **Устранено нарушение SRP в WednesdayBot и SupportBot через вынос создания компонентов в фабрики**:
  ...
  - Обновлен `WednesdayBot` для получения компонентов через DI вместо создания их внутри конструктора
  - Обновлен `SupportBot` для получения компонентов через DI вместо создания их внутри конструктора
  - Обновлены `build_bot()` и места создания `SupportBot` в `main.py` для использования фабрик компонентов
  - Обновлены тесты для передачи моков компонентов в конструкторы ботов
  - Улучшено соблюдение принципа Single Responsibility: боты больше не отвечают за создание компонентов, только за их использование
  - Улучшено соблюдение принципа Dependency Injection: все компоненты создаются в composition root (`container.py`)
  - Улучшена тестируемость: можно легко передать моки компонентов в конструкторы ботов
  - Улучшена поддерживаемость: изменения в создании компонентов централизованы в фабриках
  - Уменьшен размер классов ботов: `WednesdayBot` теперь ~150 строк вместо ~270 строк

- **Устранено дублирование кода запуска между WednesdayBot и SupportBot**:
  - Добавлен метод `_execute_startup_sequence()` в `BotLifecycleMixin` для централизованной последовательности запуска
  - Обновлены `WednesdayBot` и `SupportBot` для использования нового метода вместо дублирующейся логики
  - Улучшено соблюдение принципа DRY: общая последовательность запуска централизована в одном месте
  - Улучшена поддерживаемость: изменения в логике запуска теперь в одном месте
  - Улучшена консистентность: оба бота используют единую последовательность запуска
  - Соблюдены принципы SRP и DRY: миксин отвечает за общую логику жизненного цикла

- **Улучшена обработка исключений в chat_event_handler и устранено дублирование логики обработки ошибок генерации изображений**:
  - Добавлена обработка критических ошибок (KeyboardInterrupt, SystemExit, MemoryError, SystemError) в `chat_event_handler.py` с пробросом выше
  - Критические ошибки теперь пробрасываются выше, а не скрываются общим `Exception`
  - Добавлен метод `_handle_image_generation_errors()` в `BaseHandlers` для централизованной обработки ошибок генерации изображений
  - Обновлен `handlers_admin.py` для использования нового метода вместо дублирующейся логики
  - Улучшено соблюдение принципа DRY: логика обработки ошибок генерации изображений централизована в одном месте
  - Улучшена поддерживаемость: изменения в логике обработки ошибок теперь в одном месте
  - Улучшена надежность: критические системные ошибки правильно пробрасываются выше

- **Добавлена защита от утечек памяти в обработчике ошибок команд**:
  - Добавлен параметр `max_retries` в метод `_handle_command_errors()` с явным ограничением (по умолчанию 1, максимум 5)
  - Реализован retry-механизм только для сетевых ошибок Telegram API (TelegramError, NetworkError, TimedOut) с экспоненциальным backoff
  - Ошибки валидации, сервисного слоя и репозитория не ретраятся (как и должно быть)
  - Добавлена защита от утечек памяти: явное ограничение количества попыток, не накапливаются исключения
  - Улучшена отказоустойчивость: сетевые ошибки могут быть повторены с ограничением попыток
  - Улучшена безопасность: по умолчанию retry отключен (max_retries=1), что предотвращает двойной retry с `retry_on_connect_error`
  - Соблюдены принципы SRP и DRY: логика retry централизована и ограничена

- **Добавлены таймауты для параллельных внешних вызовов в bot-слое**:
  - Добавлен метод `_gather_with_timeout()` в `BaseHandlers` для выполнения `asyncio.gather` с таймаутами
  - Добавлены константы `CHAT_INFO_TIMEOUT_DEFAULT` (5.0с) и `CHAT_TIMEOUT_DEFAULT` (10.0с) для таймаутов
  - Обновлены все вызовы `asyncio.gather` в `handlers_admin.py` для использования таймаутов при получении информации о чатах и администраторах
  - Улучшена отказоустойчивость: защита от зависания обработчиков при проблемах с сетью
  - Улучшена производительность: предотвращение накопления незавершенных задач при зависании одного запроса
  - Соблюдены принципы DRY: логика таймаутов централизована в одном месте
  - Улучшена надежность: обработчики команд не блокируются навсегда при проблемах с Telegram API

- **Устранена протечка инфраструктуры через хранение postgres_pool и redis_client в BotServices**:
  - Удалены поля `postgres_pool` и `redis_client` из `BotServices` и `SupportBotServices`
  - Cleanup ресурсов теперь выполняется через глобальные функции `close_postgres_pool()` и `close_redis()`, что исключает необходимость хранения инфраструктурных объектов в контейнере сервисов
  - Обновлены функции `build_bot_services()` и `build_support_bot_services()` в `container.py` для удаления передачи инфраструктурных объектов
  - Обновлены тесты для удаления передачи моков `postgres_pool` и `redis_client` при создании `BotServices` и `SupportBotServices`
  - Улучшена инкапсуляция: cleanup ресурсов является внутренней деталью сервисов, а не частью публичного API контейнера
  - Улучшено соблюдение принципа Single Responsibility: `BotServices` содержит только бизнес-сервисы, а не инфраструктурные детали
  - Улучшена тестируемость: не нужно мокать пулы соединений при создании контейнера сервисов
  - Улучшена архитектурная чистота: слой `bot/` не получает доступ к деталям инфраструктуры через `services.postgres_pool` или `services.redis_client`
  - Соблюдены границы слоев: инфраструктурные объекты не просачиваются в контейнер бизнес-сервисов

- **Устранена протечка инфраструктуры через импорты в bot-слое**:
  - Удалены прямые импорты `get_logger` из `infra.logging.logger` в слое `bot/`
  - Логгер теперь внедряется через Dependency Injection во все классы bot-слоя
  - Обновлены `BaseHandlers`, `BotErrorHandler`, `WednesdayBot`, `SupportBot` для принятия логгера через конструктор
  - Обновлены все handlers (`UserHandlers`, `AdminHandlers`, `ModelHandlers`) для передачи логгера в базовый класс
  - Обновлены `build_bot()` в `container.py` и создание `SupportBot` в `main.py` для создания и передачи логгеров
  - Улучшено соблюдение принципа Dependency Inversion (DIP): bot-слой зависит от абстракции `ILogger`, а не от конкретной реализации
  - Улучшена тестируемость: можно легко подменить логгер моком в тестах
  - Улучшена архитектурная чистота: bot-слой больше не знает о деталях реализации инфраструктуры логирования
  - Соблюдены границы слоев: bot-слой не просачивается в инфраструктурный слой через импорты

### Изменено

- **Устранено дублирование кода отправки уведомлений о жизненном цикле**:
  - Добавлен универсальный метод `_send_lifecycle_notification()` в `BotLifecycleMixin` для отправки уведомлений о запуске/остановке
  - Рефакторены методы `start()` и `stop()` в `WednesdayBot` для использования нового метода
  - Рефакторены методы `start()` и `stop()` в `SupportBot` для использования нового метода
  - Устранено дублирование логики преобразования chat_id и admin_chat_id
  - Улучшено соблюдение принципа DRY: логика отправки уведомлений централизована в одном месте
  - Упрощена поддержка: изменения в логике отправки уведомлений теперь в одном месте
  - Улучшена консистентность: оба бота используют единый подход к отправке уведомлений

- **Улучшена типизация в обработке информации о пользователях**:
  - Заменено использование `hasattr()` и `getattr()` на проверку типов через `isinstance()` в методе `unmod_command()`
  - Добавлена явная обработка типов `User` и `Chat` для корректного извлечения имени и username
  - Улучшена читаемость кода: использование типизированных атрибутов вместо динамических проверок
  - Улучшена безопасность типов: mypy может лучше проверять корректность использования атрибутов
  - Улучшена поддерживаемость: код более явно показывает, какие типы ожидаются

- **Устранено избыточное логирование в методах безопасной отправки сообщений**:
  - Убрано дублирующее логирование в `_safe_reply_text()` и `_safe_reply_with_fallback()`
  - Логируются только неожиданные ошибки (не сетевые/Telegram), так как `retry_on_connect_error()` уже логирует сетевые ошибки через `log_event()`
  - Уменьшен объем логов без потери важной информации
  - Улучшена читаемость логов: устранено дублирование записей
...
- **Улучшена обработка ошибок в handlers_admin.py - заменены широкие except Exception на специфичные**:
  - Добавлена специфичная обработка ошибок валидации (`ValueError`, `TypeError`, `AttributeError`)
  - Добавлена специфичная обработка сетевых ошибок Telegram API (`TelegramError`, `NetworkError`, `TimedOut`)
  - Добавлена специфичная обработка ошибок сервисного слоя (`ServiceError`)
  - Добавлена специфичная обработка ошибок репозитория (`RepoError`)
  - Добавлена специфичная обработка ошибок генерации изображений (`ImageGenerationError`, `CircuitBreakerOpen`)
  - Критические ошибки (память, системные) теперь пробрасываются выше для правильной обработки
  - Улучшена диагностика: разные типы ошибок логируются с соответствующими уровнями (warning/error)
  - Улучшена отказоустойчивость: специфичные ошибки обрабатываются более точно
  - Улучшено соблюдение принципа "fail fast": критические ошибки не скрываются

- **Добавлена обработка ошибок в retry-механизме _safe_reply_text**:
  - Заменен декоратор `@retry_telegram` на явный вызов `retry_on_connect_error` с обработкой исключений
  - Добавлена обработка финальных ошибок с логированием (warning уровень)
  - Метод больше не пробрасывает исключения - это безопасный метод, который не должен прерывать выполнение обработчика
  - Улучшена отказоустойчивость: ошибки отправки сообщений не приводят к краху обработчика
  - Улучшена observability: все ошибки логируются для диагностики
  - Удален неиспользуемый импорт `retry_telegram`

- **Устранено дублирование кода между WednesdayBot и SupportBot**:
  - Создан миксин `BotLifecycleMixin` для общей логики жизненного цикла ботов
  - Вынесена общая логика управления состоянием (is_running, _stop_event) в миксин
  - Вынесена общая логика ожидания остановки в метод `_wait_for_stop_signal()`
  - Вынесена общая логика инициализации состояния в метод `_initialize_lifecycle_state()`
  - Вынесена общая логика остановки жизненного цикла в метод `_stop_lifecycle()`
  - Вынесена общая логика очистки ресурсов в метод `_cleanup_resources()`
  - Улучшено соблюдение принципа DRY: общая логика централизована в миксине
  - Упрощена поддержка: изменения в логике жизненного цикла теперь в одном месте

- **Приведен SupportBot к использованию AdminNotificationService**:
  - Добавлен `admin_notification_service` в `SupportBotServices`
  - Заменены прямые вызовы `bot.send_message()` на использование `AdminNotificationService`
  - Улучшена консистентность: оба бота используют единый подход к уведомлениям
  - Улучшена тестируемость: уведомления теперь можно мокировать через сервис

- **Добавлена валидация чата в SupportBot**:
  - Добавлен `BotChatAccessValidator` для проверки доступа к чату перед запуском
  - Улучшена надежность: проблемы с доступом к чату обнаруживаются на раннем этапе
  - Улучшена консистентность: оба бота используют единый подход к валидации

- **Удален warmup из SupportBot**:
  - Удален параметр `enable_warmup=True` из вызова `start_application()`
  - Упрощена логика запуска: warmup не требуется для SupportBot

- **Добавлена явная проверка успешности операций в обработчиках команд управления чатами**:
  - Добавлена проверка `result.success` в `admin_add_chat_command` и `admin_remove_chat_command`
  - Добавлено логирование успешных операций (info уровень)
  - Добавлено логирование неуспешных операций (warning уровень)
  - Улучшена observability: теперь все операции логируются для диагностики
  - Улучшена консистентность: единый подход к обработке результатов операций
  - Подготовлена основа для возможных компенсирующих действий при ошибках

- **Добавлено логирование для игнорируемых исключений в SupportBot и BotErrorHandler**:
  - Добавлено логирование временных сетевых ошибок при отправке уведомлений в SupportBot (debug уровень)
  - Добавлено логирование неожиданных ошибок при отправке уведомлений в SupportBot (warning уровень)
  - Добавлено логирование ошибок при получении списка администраторов в SupportBot
  - Добавлено логирование ошибок интеграции Sentry в BotErrorHandler (warning уровень)
  - Добавлен fallback логирования через print в stderr для критических ошибок в BotErrorHandler
  - Улучшена observability: теперь все ошибки логируются для диагностики
  - Сохранена безопасность: исключения не пробрасываются, чтобы не прерывать выполнение

- **Добавлена валидация входных данных в обработчиках команд управления чатами**:
  - Добавлена проверка диапазона для chat_id в `admin_add_chat_command` и `admin_remove_chat_command`
  - Валидация проверяет, что chat_id находится в допустимом диапазоне int64 и не равен нулю
  - Улучшена обработка ошибок валидации с информативными сообщениями
  - Улучшена надежность: предотвращаются ошибки при передаче некорректных значений
  - Улучшена явность валидации: проверки выполняются до вызова сервисного слоя

- **Улучшена обработка ошибок в ChatEventHandler**:
  - Разделена обработка временных (retry) и постоянных ошибок при отправке приветствия
  - Добавлена специфичная обработка ServiceError для операций добавления/удаления чатов
  - Улучшена диагностика: различаются сетевые ошибки и критические ошибки
  - Улучшено логирование: временные ошибки логируются как warning, критические как error
  - Улучшено соблюдение принципов обработки ошибок: специфичные исключения обрабатываются отдельно

- **Устранено дублирование логики обработки ошибок отправки сообщений**:
  - Заменены повторяющиеся паттерны `try/except Exception + logger.error` на использование `_safe_reply_with_fallback()` из `BaseHandlers`
  - Улучшено соблюдение принципа DRY: общая логика обработки ошибок централизована в базовом классе
  - Упрощена поддержка: изменения в логике обработки ошибок теперь в одном месте
  - Улучшена согласованность обработки ошибок между разными обработчиками
  - Обновлены `handlers_user.py`, `handlers_models.py` и `handlers_admin.py` для использования единого подхода

- **Добавлено логирование для игнорируемых исключений**:
  - Добавлено логирование ошибок при отправке fallback сообщений в `_safe_reply_with_fallback()`
  - Добавлено логирование ошибок при удалении статусных сообщений в `_safe_delete_message()`
  - Улучшена observability: теперь все ошибки логируются для диагностики
  - Сохранена безопасность: исключения не пробрасываются, чтобы не прерывать выполнение команды

- **Вынесена логика получения информации о чатах в сервисный слой**:
  - Создан `ChatInfoService` в слое `app/` для инкапсуляции логики работы с Telegram Bot API
  - Методы `_get_chat_info_safe()` и `_get_chat_safe()` в `AdminHandlers` теперь делегируют выполнение в сервис
  - Улучшено соблюдение архитектурных границ: обработчики не работают напрямую с Telegram Bot API
  - Улучшена тестируемость: не требуется мокировать `bot.get_chat()` в обработчиках
  - Улучшено соблюдение принципа единственной ответственности: обработчики только валидируют входные данные и делегируют бизнес-логику
  - Устранено дублирование логики получения информации о чатах

- **Устранен прямой доступ к context.application в обработчиках**:
  - Удалена fallback логика прямого управления жизненным циклом PTB Application из `stop_command`
  - Обработчик теперь использует только `bot_controller` через DI для остановки бота
  - Добавлена явная обработка случая, когда `bot_controller` недоступен
  - Улучшено соблюдение принципа единственной ответственности: обработчики не управляют жизненным циклом приложения
  - Улучшена тестируемость: не требуется мокировать `context.application`
  - Улучшено соблюдение архитектурных границ: вся логика жизненного цикла делегируется в сервисный слой
...
- **Замена dict[str, Any] на TypedDict для метаданных статусных сообщений**:
  ...
  - Обновлена сигнатура метода `IBotController.stop()` для использования типизированных метаданных
  - Заменены `isinstance(..., dict)` на проверку на `None` и `.get()` на прямой доступ к полям
  - Улучшена типобезопасность: статические анализаторы могут проверять правильность использования полей
  - Улучшена читаемость: явная структура данных вместо неопределенного словаря
  - Соответствие best practices: использование TypedDict для структурированных данных

- **Замена assert на явную проверку конфигурации в WednesdayBot и SupportBot**:
  - Заменен `assert` на явную проверку с поднятием `ValueError` для проверки токена Telegram бота
  - Улучшена надежность: проверки конфигурации работают даже с флагом оптимизации `-O`
  - Улучшено сообщение об ошибке: добавлено указание проверить конфигурацию
  - Соответствие best practices: использование явных исключений вместо assert для валидации конфигурации

- **Добавлены хелпер-методы в BaseHandlers для устранения дублирования кода**:
  - Добавлен метод `_safe_delete_message()` для безопасного удаления сообщений с обработкой ошибок
  - Добавлен метод `_safe_reply_text_with_error_logging()` для безопасной отправки сообщений с retry-логикой и логированием ошибок
  - Заменено дублирование кода в `handlers_admin.py` и `handlers_user.py` на использование новых хелпер-методов
  - Улучшено соблюдение принципа DRY: общая логика вынесена в базовый класс
  - Улучшена поддерживаемость: изменения в логике обработки ошибок теперь в одном месте
  - Улучшена читаемость: код хендлеров стал более декларативным

- **Устранена циклическая зависимость через services.bot_controller**:
  - Удалена установка `self.services.bot_controller = self` из конструктора `WednesdayBot`
  - Установка `bot_controller` теперь происходит только в composition root (`container.py`)
  - Устранено нарушение принципа Dependency Injection: зависимости не создаются внутри класса
  - Улучшена тестируемость: можно легко подменить `bot_controller` моком в тестах
  - Соблюдены границы слоёв: бот-слой не устанавливает зависимости в сервисы

- **Устранено нарушение инкапсуляции при остановке бота**:
  - Добавлен параметр `shutdown_metadata` в метод `IBotController.stop()`
  - Хендлеры больше не обращаются напрямую к внутреннему полю `pending_shutdown_edit`
  - Метаданные для статусного сообщения передаются через параметр метода
  - Улучшена инкапсуляция: внутреннее состояние контроллера скрыто от хендлеров
  - Упрощен рефакторинг: изменение структуры контроллера не требует изменений в хендлерах

- **Замена assert на явную валидацию типа в обработчиках**:
  - Создана функция `require_bot_services()` для проверки типа сервисов
  - Заменены `assert` проверки на явные проверки с поднятием `TypeError` в `AdminHandlers`, `UserHandlers`, `ModelHandlers`
  - Улучшена надежность: проверки типа работают даже с флагом оптимизации `-O`
  - Улучшена типобезопасность: mypy может корректно выполнять type narrowing после проверки
  - Соответствие best practices: использование явных исключений вместо assert для валидации входных данных

- **Уточнение обработки исключений в AdminHandlers**:
  - Заменены широкие `except Exception` на специфичные типы исключений в методах `_get_chat_info_safe()` и `_get_chat_safe()`
  - Добавлена обработка ошибок валидации (ValueError, TypeError, AttributeError) отдельно от Telegram API ошибок
  - Добавлено `exc_info=True` для логирования неожиданных ошибок в местах, где `Exception` оправдан (отправка сообщений, fallback логика)
  - Улучшена диагностика: логирование с полным стеком для неожиданных ошибок
  - Улучшена отказоустойчивость: специфичные исключения обрабатываются корректно, неожиданные логируются и не маскируются

- **Удален неиспользуемый метод _check_chat_access() из WednesdayBot**:
  - Удален мертвый код: метод `_check_chat_access()` не использовался в коде
  - Вместо него используется `chat_validator.validate_chat_access()` для проверки доступа к чату
  - Обновлен docstring метода `start()` для отражения фактического использования
  - Улучшено соблюдение принципа YAGNI: удален неиспользуемый код
  - Упрощена поддержка: меньше кода для поддержки

- **Удален неиспользуемый rate_limiter из SupportBot**:
  - Удалено создание `RateLimiter` в конструкторе `SupportBot`, так как он не используется
  - Удален импорт `RateLimiter`
  - Обновлены комментарии, убрав упоминание rate limiting
  - Исправлено нарушение DI: сервисы должны создаваться в composition root, а не внутри bot-слоя
  - Улучшено соблюдение принципа YAGNI: удален мертвый код

- **Добавлен вызов cleanup() в SupportBot.stop() для корректного освобождения ресурсов**:
  - Добавлен вызов `services.cleanup()` в блоке `finally` метода `stop()` для гарантированного закрытия ресурсов
  - Исправлена потенциальная утечка ресурсов (Redis, PostgreSQL pool) при остановке SupportBot
  - Улучшено управление ресурсами: ресурсы закрываются даже при ошибках остановки
  - Обеспечена консистентность с `WednesdayBot.stop()`: оба бота используют одинаковый паттерн с `finally` блоком
  - Гарантированное освобождение ресурсов: cleanup() выполняется всегда, независимо от успешности остановки

### Рефакторинг

- **Улучшение типизации в AdminHandlers**:
  - Заменен `bot: object` на `bot: Bot` в методах `_get_chat_info_safe()` и `_get_chat_safe()`
  - Заменен возвращаемый тип `-> object | None` на `-> Chat | None` в методе `_get_chat_safe()`
  - Убраны `# type: ignore` комментарии благодаря правильной типизации
  - Улучшена читаемость кода и поддержка IDE (автодополнение, проверка типов)
  - Улучшена безопасность типов: mypy может проверить корректность использования методов
...
- **Вынос создания сервисов из bot-слоя в composition root**:
  ...
  - Улучшено соблюдение принципа Dependency Injection: все сервисы создаются в composition root
  - Улучшена тестируемость: зависимости можно легко подменить в тестах
  - Улучшено соблюдение Single Responsibility Principle: `WednesdayBot` не отвечает за создание сервисов
  - Устранено нарушение архитектурных границ: bot-слой больше не зависит от деталей создания сервисов

- **Добавление таймаутов для асинхронных операций get_chat()**:
  - Создан метод `_get_chat_safe()` в `AdminHandlers` для безопасного получения полного объекта чата с таймаутом
  - Заменен прямой вызов `await context.bot.get_chat(admin_id)` на использование `_get_chat_safe()` с таймаутом 10 секунд
  - Оптимизирован код получения списка администраторов: используется параллельная обработка с `asyncio.gather()`
  - Улучшена надежность: таймауты предотвращают зависания при проблемах с сетью
  - Улучшена производительность: параллельные запросы ускоряют получение списка администраторов
  - Улучшена обработка ошибок: добавлена обработка `asyncio.TimeoutError` и других исключений
  - Исправлена проблема на строке 974 в `handlers_admin.py`

- **Оптимизация получения информации о чатах: параллельные запросы вместо последовательных**:
  - Создана вспомогательная функция `_get_chat_info_safe()` в `AdminHandlers` для безопасного получения информации о чате
  - Заменены последовательные вызовы `get_chat()` на параллельные с использованием `asyncio.gather`
  - Добавлен таймаут (5 секунд) для каждого запроса информации о чате
  - Улучшена производительность: при большом количестве чатов операция выполняется значительно быстрее
  - Улучшен UX: пользователь получает ответ быстрее благодаря параллельной обработке
  - Улучшена обработка ошибок: добавлена обработка `asyncio.TimeoutError` и других исключений
  - Обновлены два места в коде: строки 253-260 и 632-639 в `handlers_admin.py`

- **Оптимизация retry-механизма: устранение потенциальной утечки памяти**:
  - Вынесены классы `_MockOutcome` и `_MockRetryState` за пределы функции `retry_on_connect_error`
  - Классы теперь создаются один раз при импорте модуля, а не на каждой итерации цикла
  - Улучшена производительность: устранено создание классов внутри цикла retry
  - Улучшена эффективность использования памяти: mock-объекты не накапливаются при частых вызовах
  - Добавлена документация для классов `_MockOutcome` и `_MockRetryState`
  - Упрощен код функции `retry_on_connect_error`: убрано дублирование определения классов
...
- **Добавлена обработка событий чата в SupportBot для синхронизации списка чатов**:
  - Добавлен параметр `chats_repo: IChatsRepo` в конструктор `SupportBot` для доступа к репозиторию чатов
  - Обновлен `BotServices` в SupportBot: теперь использует `chats_repo` вместо `None`
  - Добавлен `ChatEventHandler` в SupportBot для обработки событий добавления/удаления бота из чатов
  - Зарегистрирован `ChatMemberHandler` в `setup_handlers()` SupportBot для обработки событий чата
  - Обновлен `main.py`: создается `ChatsRepo` и передается в SupportBot через DI
  - Исправлена проблема: если бота удалят из чата, пока работает SupportBot, событие теперь обрабатывается
  - Улучшена синхронизация: изменения чатов обрабатываются и в SupportBot, и в WednesdayBot
  - Улучшена консистентность: список чатов в PostgreSQL всегда актуален, независимо от того, какой бот работает
  - Предотвращены ошибки: WednesdayBot больше не будет пытаться отправлять в несуществующие чаты
  - Соблюдение границ слоев: SupportBot получает ChatsRepo через DI, не создает его сам
  - Переиспользование кода: SupportBot использует тот же `ChatEventHandler`, что и WednesdayBot

### Рефакторинг

- **Вынос обработки событий чата в отдельный компонент ChatEventHandler**:
  - Создан `ChatEventHandler` (`src/bot/chat_event_handler.py`) для обработки событий изменения статуса бота в чатах
  - Вынесен метод `on_my_chat_member()` из `WednesdayBot` в `ChatEventHandler`
  - Улучшена обработка ошибок: добавлена детальная обработка ошибок при добавлении/удалении чатов
  - Обновлен `WednesdayBot`: использует `ChatEventHandler` через DI вместо прямого обработчика
  - Обновлен `setup_handlers()`: регистрирует обработчик через `ChatEventHandler.on_my_chat_member`
  - Улучшено соблюдение SRP: `WednesdayBot` больше не содержит логику обработки событий чата
  - Улучшена модульность: обработка событий чата изолирована в отдельном компоненте
  - Улучшена тестируемость: `ChatEventHandler` можно тестировать независимо от `WednesdayBot`
  - Соблюдение границ слоев: обработчик в bot-слое, использует сервисы через DI
...
- **Оптимизация основного цикла бота: замена busy-wait на asyncio.Event**:
  - Заменен цикл `while self.is_running: await asyncio.sleep(0.1)` на использование `asyncio.Event` в `wednesday_bot.py`
  - Добавлен `self._stop_event = asyncio.Event()` в `__init__()` для ожидания сигнала остановки
  - Обновлен метод `start()`: теперь использует `await self._stop_event.wait()` вместо busy-wait цикла
  - Обновлен метод `stop()`: теперь вызывает `self._stop_event.set()` для разблокировки ожидания
  - Улучшена производительность: устранено потребление CPU в простое (busy-wait цикл с sleep(0.1))
  ...
  - Улучшена отзывчивость: остановка бота происходит мгновенно через Event.set()
  - Соответствие best practices: использование Event для координации между корутинами
  - Исправление проблемы из архитектурного аудита bot-слоя (бесконечный цикл в start())

- **Устранение нарушения архитектурных границ: передача AdminsRepo через DI в SupportBot**:
  - Удален прямой импорт `AdminsRepo` из `infra.repos` в `support_bot.py`
  - Удалено создание `AdminsRepo` внутри `SupportBot.__init__()` (нарушение границ слоев)
  - Добавлен параметр `admins_repo: IAdminsRepo` в конструктор `SupportBot` для передачи через DI
  - Обновлен `main.py`: `AdminsRepo` создается вне bot-слоя и передается в `SupportBot` через конструктор
  - Улучшена архитектурная чистота: bot-слой не создает компоненты из infra-слоя напрямую
  - Улучшена тестируемость: теперь можно легко подменить `admins_repo` в тестах через конструктор
  - Соответствие принципу Dependency Inversion: bot-слой зависит от абстракции (`IAdminsRepo`), а не от конкретной реализации
  - Соответствие границам слоев: bot-слой не знает о деталях инфраструктурного слоя, использует только сервисы через DI
  - Исправление проблемы из архитектурного аудита bot-слоя (нарушение границ слоев)
...
- **Устранение нарушений архитектурных границ в bot слое: использование DI вместо прямых импортов из infra/**:
  ...
  - Исправлен `wednesday_bot.py`: заменены все прямые создания `AdminsRepo` на использование `self.services.admins_repo` через DI (4 места)
  - Исправлен `support_bot.py`: `AdminsRepo` создается в `__init__` и передается через `BotServices` для соблюдения DI
  - Улучшена архитектурная чистота: слой `bot/` больше не зависит напрямую от конкретных реализаций из `infra/`
  - Улучшена тестируемость: теперь можно легко подменить `admins_repo` через `BotServices` в тестах
  - Соответствие принципу Dependency Inversion: bot-слой зависит от абстракций (`IAdminsRepo` через `BotServices`), а не от конкретных реализаций
  - Соответствие границам слоев: bot-слой не знает о деталях инфраструктурного слоя, использует только сервисы через DI
  - Исправление критических проблем из архитектурного аудита bot-слоя

- **Унификация стандарта TypedDict vs Dataclass: добавление документации стандарта**:
  - Добавлены комментарии о стандарте использования TypedDict и Dataclass в DTO
  - Обновлен `FrogRequestResult`: добавлен комментарий о стандарте (TypedDict для неизменяемых структур)
  - Обновлен `CommandResult`: добавлен комментарий о стандарте (Dataclass для мутабельных DTO)
  - Обновлен `ChatInfo`: добавлен комментарий о стандарте (Dataclass для мутабельных DTO)
  - Обновлен `DispatchResult`: добавлено упоминание стандарта в существующий комментарий
  - Установлен единый стандарт: Dataclass для мутабельных DTO, TypedDict для неизменяемых структур данных
  - Улучшена читаемость: явная документация выбора между TypedDict и Dataclass
  - Улучшена консистентность: единый подход к выбору типа DTO во всех модулях
  - Соответствие принципу явности: стандарт документирован в коде
  - Соответствие архитектуре: изменение не нарушает слои, остается в app layer
  - Исправление технического долга из архитектурного аудита app-слоя

- **Улучшение типизации метрик: создание TypedDict для MetricsSummary**:
  - Создан `MetricsSummary` TypedDict в `admin_dashboard_builders.py` для типизации структуры метрик производительности
  - Заменено использование `dict[str, Any]` на `MetricsSummary` в `StatusData` dataclass
  - Обновлен метод `_format_metrics_text()` в `StatusMessageBuilder`: параметр типизирован как `MetricsSummary | None`
  - Добавлен импорт `MetricsSummary` в `admin_dashboard_service.py`
  - Улучшена типобезопасность: mypy может проверять структуру метрик на этапе компиляции
  - Улучшена читаемость: явная структура метрик вместо словаря с Any
  - Улучшена документация: TypedDict явно описывает все поля метрик
  - Соответствие принципу явности: типизированная структура вместо неопределенного словаря
  - Соответствие архитектуре: изменение не нарушает слои, остается в app layer
  - Исправление технического долга из архитектурного аудита app-слоя

- **Устранение утечки инфраструктурных деталей: удаление явной типизации asyncpg.Connection**:
  - Удален импорт `asyncpg` из `database_operations_service.py`
  - Удалена явная типизация `connection: asyncpg.Connection` в методах `record_dispatch_success()` и `reserve_and_finalize_dispatch()`
  - Заменено на использование типа из протокола: `connection = uow.connection` (тип выводится из `IDatabaseUnitOfWork[ConnectionType]`)
  - Улучшена архитектурная чистота: слой `app/` больше не зависит от конкретной реализации БД (asyncpg)
  - Улучшена гибкость: теперь можно заменить PostgreSQL на другую БД без изменения app-слоя
  - Улучшена тестируемость: не нужно мокать asyncpg, достаточно мока протокола
  - Соответствие принципу Dependency Inversion: app-слой зависит от абстракций (протоколов), а не от конкретных реализаций
  - Соответствие границам слоев: app-слой не знает о деталях инфраструктурного слоя
  - Исправление критической проблемы из архитектурного аудита app-слоя
...
- **Устранение дублирования логики проверки dispatch: унификация через batch-проверку**:
  ...
  - Добавлены helper-функции в `dispatch_targets_helper.py`:
    - `check_dispatch_status_batch()` - унифицированная функция для batch-проверки статуса отправки
    - `are_all_dispatched()` - проверка, отправлено ли во все чаты
    - `get_undispatched_chats()` - получение множества неотправленных чатов
  - Обновлен `TargetPreparationService.is_already_dispatched_for_all()`: использует batch-проверку вместо последовательных запросов
  - Обновлен `process_targets_with_registry_check()`: использует batch-проверку для оптимизации обработки целевых чатов
  - Устранено дублирование логики проверки: оба места используют единую функцию `check_dispatch_status_batch()`
  - Оптимизирована производительность: один batch-запрос к БД вместо N отдельных запросов (особенно эффективно при большом количестве чатов)
  - Улучшена консистентность: одинаковая логика проверки в обоих местах использования
  - Улучшена масштабируемость: эффективная обработка множества чатов за один запрос
  - Соответствие принципу DRY: устранено дублирование логики проверки
  - Соответствие архитектуре: нет утечек слоев, app использует протоколы из shared, infra реализует протоколы
...
- **Устранение избыточных опциональных зависимостей: обязательные критичные зависимости и документация опциональных**:
  ...
  - Задокументированы опциональные зависимости в `ImageService`: `caption_service` - не критична, используется только для улучшения (подписи)
  - Задокументированы опциональные зависимости в `ImageGenerationCoordinator`: `circuit_breaker`, `image_cache`, `metrics` - опциональные cross-cutting concerns с описанием поведения при None
  - Задокументированы опциональные зависимости в `AdminDashboardService`: `usage`, `chats`, `metrics` - опциональны, при None отображается "N/A" или "Не настроены"
  - Обновлен DI-контейнер: порядок параметров в `ImageService` обновлен (обязательный `image_storage` перед опциональным `caption_service`)
  - Улучшена ясность контрактов сервисов: явно указано, какие зависимости критичны, а какие опциональны
  - Улучшена документация: описано поведение при None для всех опциональных зависимостей
  - Соответствие принципу явности: контракты сервисов теперь более понятны

- **Устранение нарушения Single Responsibility: выделение форматирования уведомлений в Builder классы**:
  - Создан модуль `admin_notification_builders.py` с билдерами для форматирования уведомлений
  - Создан `GenerationErrorNotificationBuilder` для форматирования сообщений об ошибках генерации
  - Создан `DispatchErrorNotificationBuilder` для форматирования сообщений об ошибках рассылки
  - Добавлены dataclass `GenerationErrorData` и `DispatchErrorData` для передачи данных в билдеры
  - Рефакторинг `AdminNotificationService`: удалены методы форматирования (`_format_*`), форматирование делегировано билдерам
  - Обновлен DI-контейнер: создаются билдеры и передаются в `AdminNotificationService`
  - Улучшено разделение ответственности: `AdminNotificationService` отвечает только за координацию отправки, билдеры - за форматирование
  - Улучшена тестируемость: форматирование и отправку можно тестировать отдельно
  - Улучшена переиспользование: билдеры можно использовать в других каналах (email, webhook)
  - Улучшена расширяемость: новые типы уведомлений добавляются новыми билдерами без изменения существующих
  - Соответствие принципу Single Responsibility: каждый класс отвечает за свою область
  - Соответствие паттерну проекта: используется тот же подход, что и в `StatusMessageBuilder` и `ModelsListMessageBuilder`
  - Соответствие принципу Dependency Inversion: билдеры инжектируются через конструктор, можно мокировать

- **Реализация оптимистической брони для атомарности отправки и регистрации dispatch**:
  - Добавлен метод `try_reserve_dispatch()` в протокол `IDispatchRegistry` для атомарного захвата права на отправку
  - Реализован метод `try_reserve_dispatch()` в `DispatchRegistry`: использует `INSERT ... ON CONFLICT DO NOTHING` для атомарного бронирования
  - Добавлен метод `reserve_and_finalize_dispatch()` в `DatabaseOperationsService`: выполняет бронирование и финализацию (usage, metrics) в одной транзакции
  - Обновлен метод `_send_single_image()` в `DispatchDeliveryService`: использует оптимистическую бронь перед отправкой изображения
  - Устранена проблема неатомарности: отправка изображения и регистрация теперь выполняются атомарно через бронирование
  - Защита от гонок: проверка и создание записи выполняются в одной атомарной операции на уровне БД
  - Улучшена идемпотентность: повторные вызовы безопасны, дубликаты отправок исключены
  - Fail-safe поведение: если бронь получена, но отправка падает, при следующем запуске будет пропуск (безопаснее, чем дубликат)
  - Соответствие принципу ACID: транзакционность гарантируется на уровне БД
  - Улучшена надежность: исключены race conditions между проверкой и регистрацией отправки
...
- **Рефакторинг ImageService: разделение ответственности на координаторы**:
  ...
  - Рефакторинг `ImageService` до тонкого оркестратора (промпт → подпись → генерация → сохранение)
  - Улучшено разделение ответственности: каждый координатор отвечает за свою область
  - Улучшена тестируемость: координаторы можно тестировать изолированно
  - Обновлен DI-контейнер для создания координаторов
  - Соответствие принципам SOLID: Single Responsibility Principle соблюдается на всех уровнях
...
- **Улучшение архитектуры admin-сервисов: использование протоколов вместо конкретных реализаций**:
  ...
  - В `container.py` функции сборки admin-сервисов используют протокол `IAdminsRepo` вместо конкретной реализации `AdminsRepo`
  - Соответствие принципу Dependency Inversion: зависимости от абстракций, а не от конкретных реализаций

- **Рефакторинг admin-сервисов: централизация админских операций и соблюдение границ слоёв**:
  - Создан `AdminAccessService` для централизованной проверки прав администратора и главного администратора
  - Создан `AdminCommandService` для координации всех админских команд (add_chat, remove_chat, set_frog_threshold, set_frog_used, mod, unmod)
  - Расширен `AdminNotificationService` для поддержки всех типов уведомлений (generation_failure, dispatch_failure)
  - Рефакторинг `handlers_admin.py`: убраны прямые вызовы репозиториев, все операции выполняются через app-слой
  - Устранено нарушение границ слоёв: handlers больше не обращаются напрямую к infra-репозиториям
  - Централизована проверка прав администратора через `AdminAccessService`
  - Улучшена тестируемость: вся бизнес-логика админских команд вынесена в сервисы
  - Добавлено исключение `AccessDeniedError` для явной обработки ошибок доступа
  - Соответствие принципам SOLID: улучшено разделение ответственности, упрощены зависимости
  - Единообразие архитектуры: все админские операции следуют единому паттерну через app-слой
...
- **Рефакторинг dispatch сервисов: объединение DispatchExecutionService и FallbackService**:
  ...
  - Упрощена архитектура: удалены циклические зависимости между сервисами
  - Улучшена обработка ошибок: вынесена в отдельные методы `_handle_messaging_error`, `_handle_app_error`, `_notify_error_and_update_metrics`
  - Обновлен `DispatchService` для использования нового `DispatchDeliveryService`
  - Методы обработки ошибок генерации и неожиданных ошибок перенесены в `DispatchService`
  - Удалены устаревшие сервисы `DispatchExecutionService` и `FallbackService`
  - Обновлен DI-контейнер для использования новой архитектуры
  - Соответствие принципам SOLID: улучшено разделение ответственности, упрощены зависимости
...
- **AdminNotificationService для уведомления администраторов**:
  - Создан сервис `AdminNotificationService` в `src/app/admin_notification_service.py` для уведомления администраторов об ошибках
  - Реализован метод `notify_generation_failure` для уведомления об ошибках генерации изображений
  - Добавлена логика форматирования сообщений с обрезкой длинных трейсов и проверкой длины сообщений
  - Реализована обработка ошибок отправки с логированием без прерывания процесса уведомления других администраторов
  - Определен локальный протокол `IAdminsRepo` для зависимости от репозитория администраторов

- **FrogProcessingService для обработки команды /frog**:
  - Создан сервис `FrogProcessingService` в `src/app/frog_processing_service.py` для обработки запросов генерации жабы
  - Реализован метод `process_frog_request` для координации генерации, отправки, обновления usage и удаления статусных сообщений
  - Добавлена fallback-логика при ошибках генерации с отправкой дружелюбного сообщения и случайного изображения из архива
...
- **Исправление реализации рекомендаций из INFRA_LAYER_AUDIT.md**:
  - Добавлен `FrogProcessingService` в контекст сервисов Celery (`get_services_context`) для правильного применения Dependency Injection
  - Обновлена функция `send_frog_manual` для использования `FrogProcessingService` из контекста вместо ручного создания зависимостей
...
- **Завершение рефакторинга глобальных пулов и клиентов**:
  - Обновлена функция `_ensure_pools_initialized()` для возврата пулов явно вместо использования приватных функций
  - Обновлена функция `get_services_context()` для использования возвращаемых пулов вместо `_get_postgres_pool()` и `_get_redis()`
  - Обновлена функция `get_pool_metrics()` для обязательного параметра `pool` (удален fallback на глобальный пул)
  - Обновлена функция `update_pool_metrics()` для обязательного параметра `pool`
  - Обновлен `healthcheck.py` для работы с обновленной `get_pool_metrics()` (теперь всегда возвращает PoolMetrics)
  - Устранена скрытая зависимость от глобальных синглтонов в `celery/context.py` - пулы теперь получаются явно после инициализации
  - Улучшена типобезопасность и соответствие принципам Dependency Injection
...
- **Явная инициализация и валидация инфраструктурных зависимостей**:
  - Добавлен публичный тип `RedisClient` в `src/infra/redis/redis_client.py` для единого типа Redis-клиента
  - Добавлен метод `_init_and_validate_infrastructure()` в `BotRunner` для централизованной инициализации и валидации Postgres и Redis
  - Реализована валидация Postgres через выполнение тестового запроса после инициализации пула
  - Реализована валидация Redis через ping после инициализации клиента
  - Добавлено структурированное логирование событий инициализации инфраструктуры через `log_event`

### Изменено

- **Рефакторинг инициализации инфраструктуры в main.py**:
  - Обновлен метод `run()` для использования нового метода `_init_and_validate_infrastructure()`
  - Удалены методы `_init_redis_if_configured()` и `_init_postgres_if_configured()` - заменены на единый метод валидации
  - Обновлен вызов `build_bot()` для явной передачи `redis_client` в качестве параметра
  - Обновлены функции `build_bot()` и `build_bot_services()` в `container.py` для приема `redis_client` явно
...
- **Расширение контекста сервисов в Celery**:
  - Обновлена функция `get_services_context()` в `celery/context.py` для включения `postgres_pool` и `redis_client` в контекст
  - Добавлена документация для новых полей контекста (`postgres_pool` и `redis_client`)
  - Обновлены все места использования `get_postgres_pool()` в `celery/tasks.py` для использования пула из контекста
  - Обновлена функция `send_frog_manual` для получения пула из контекста вместо прямого вызова `get_postgres_pool()`
...
- **Валидация параметров и улучшенное логирование**:
  - Добавлена валидация обязательных параметров в функциях `build_*` (`build_bot`, `build_bot_services`, `build_image_stack`, `build_admin_dashboard_service`)

- **Реализация рекомендаций из INFRA_LAYER_AUDIT.md по явной передаче пулов через DI**:
  - Добавлен параметр `pool` в функцию `record_metric()` для явной передачи пула подключений PostgreSQL
  - Добавлен параметр `pool` в функцию `ensure_schema()` для явной передачи пула при инициализации схемы БД (теперь обязательный)
  - Добавлен обязательный параметр `pool` в функции `get_daily_generation_stats()` и `get_top_prompts()` для явной передачи пула (удален fallback на глобальный пул)
  - Обновлен `main.py` для передачи пула явно в `ensure_schema()` после инициализации инфраструктуры
  - Обновлен `celery/context.py` для передачи пула явно в `ensure_schema()` при инициализации пулов в worker процессе
  - Обновлен `Metrics.increment_cache_hit()` для передачи пула явно в `record_metric()`
  - Добавлен параметр `postgres_pool` в `MetricsRecorder` для передачи пула в `record_metric()`
  - Обновлен `container.py` для передачи пула в `MetricsRecorder` при создании
  - Рефакторинг `healthcheck.py` - убраны fallback на глобальные функции, используется только `app.state`
  - Обновлен `main.py` для установки пулов в `app.state` после инициализации инфраструктуры
  - Удалены fallback на глобальные пулы из `record_metric()` и `ensure_schema()` - теперь требуется явная передача через DI
  - Добавлены deprecation warnings в `_get_postgres_pool()` и `_get_redis()` для предупреждения об использовании устаревших функций
  - Обновлен тестовый файл `tests/helpers/utils/postgres_schema.py` для передачи пула в `ensure_schema()`
  - Все функции `build_*` теперь проверяют, что обязательные параметры не равны `None`, и выбрасывают `ValueError` с понятным сообщением
  - Логирование в критических местах уже реализовано в `main.py` через структурированные события (`log_event`)
  - Логирование fallback и deprecated методов уже реализовано с уровнем WARNING

- **Удаление публичных функций get_postgres_pool() и get_redis()**:
  - Публичные функции `get_postgres_pool()` и `get_redis()` переименованы в приватные `_get_postgres_pool()` и `_get_redis()`
  - Все использования публичных функций заменены на приватные или явную передачу через DI
  - Обновлены все модули для использования приватных функций или явной передачи зависимостей
  - Обновлены все тесты для использования приватных функций

- **Удаление всех fallback-механизмов для полного соответствия DI**:
  - `SupportBot`: параметр `postgres_pool` теперь обязательный (удален `| None = None`), удален fallback на `_get_postgres_pool()`
  - `DatabaseUnitOfWork`: параметр `pool` теперь обязательный (удален `| None = None`), удален fallback `pool or _get_postgres_pool()`
  - `MetricsRecorder`: параметр `metrics` теперь обязательный (удален `| None = None`), удален fallback `metrics or Metrics(pool=_get_postgres_pool())`
  - `DispatchRegistry.register()`: удален fallback на `_get_postgres_pool()`, используется `self._pool` из конструктора
  - Все конструкторы теперь требуют обязательные параметры и выбрасывают `ValueError` при `None`
  - Обновлен `container.py` для явного создания `Metrics` перед передачей в `MetricsRecorder`
...
- **Интеграция FailedCacheQueue в container.py и восстановление при старте**:
  ...
  - Добавлен вызов `restore_from_persistent_queue` при старте бота в `main.py` для восстановления очереди из Redis
  - Восстановление очереди выполняется безопасно с обработкой ошибок и не прерывает запуск бота
...
- **ModelManagementService для управления моделями ML-клиентов**:
  ...
  - Логика выбора моделей вынесена из клиентов в app-слой, что улучшает разделение ответственности
  - Добавлен `ModelManagementService` в `BotServices` для использования в обработчиках
...
- **Вынесена логика управления моделями в app-слой**:
