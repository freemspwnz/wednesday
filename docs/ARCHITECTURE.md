...
**ВАЖНО:** Не используйте `_get_postgres_pool()` или `_get_redis()` напрямую!
...
- **Явная передача инфраструктурных зависимостей:** Все функции сборки (`build_bot`, `build_bot_services`, `build_image_stack`, `build_admin_dashboard_service`) принимают `db_pool` и `redis_client` явно через параметры, а не через глобальные функции. Публичные функции `get_postgres_pool()` и `get_redis()` были удалены и заменены на приватные `_get_postgres_pool()` и `_get_redis()` для внутреннего использования только в модулях, где они определены. Это обеспечивает прозрачность зависимостей и упрощает тестирование.
