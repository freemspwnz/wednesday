...
class WednesdayBot:
    ...
    async def start(self) -> None:
        ...
        Инициализирует приложение, настраивает обработчики команд, запускает
        планировщик (если используется старый TaskScheduler), начинает polling
        и отправляет уведомления о запуске. Запускает основной цикл ожидания,
        который работает до получения сигнала остановки.
